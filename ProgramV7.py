...
conn = sqlite3.connect('myfooddatabaseV4.db')
...
client = InferenceHTTPClient(
    ...
    api_key="Please input the api key here"
    )
    ...
    if x is True:
        ...
        def submit():
            ...
            if not selected_ingredient_ids:
                ...
                speak("No Ingredients. Please select some ingredients first!")
                return
            if y is True:
                gen(selected_ingredient_ids,selected_ingredient_names)
...
recipetext=0
def gen(x,y):
    print(f"Finding recipes for ingredient IDs: {x}")
    ...
    # Display results
    result_window = tk.Toplevel(root,bg="#111111")
    result_window.title("Recipe Results")
    result_window.geometry("600x400")
    
    # Create scrollable text area
    text_frame = tk.Frame(result_window,bg="#111111")
    text_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    
    scrollbar = tk.Scrollbar(text_frame)
    scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    
    result_text = tk.Text(text_frame, yscrollcommand=scrollbar.set, wrap=tk.WORD,fg="#FFFFFF",bg="#111111")
    result_text.pack(fill=tk.BOTH, expand=True)
    scrollbar.config(command=result_text.yview)
    ...
    if matching_recipes:
        ...
        for recipe in matching_recipes:
            ...
            for i, col in enumerate(f_columns):
                ...
                if ingredient_fid and ingredient_fid in x:
        ...
        result_text.insert(tk.END, f"Your selected ingredients: {y}\n")
        result_text.insert(tk.END, f"Your selected ingredient IDs: {x}")
        c="2.0"
    global recipetext
    recipetext=result_text.get("1.0", c)
    result_text.config(state=tk.DISABLED)  # Make read-only

    def rbut_click():
        S[2]="Pronounce the found recipes"
        doubleclick(0,0,9)
        if S[3] is True:
            speak(recipetext)
    precipe_button=tk.Button(result_window, text="Pronounce the found recipes", command=rbut_click ,bg="#FFFFFF",font=("Arial",12))
    precipe_button.pack(pady=5,side=tk.LEFT)
    close_button = tk.Button(result_window, text="Close", command=result_window.destroy,bg="#FFFFFF",font=("Arial",12))
    close_button.pack(pady=5,side=tk.LEFT)
...
def retrieve(x):
    if x is True:
        storedID=[]
        storedName=[]
        cursor2.execute("select distinct(FID) from expiry")
        rows=cursor2.fetchall()
        if not rows:
            speak("The storage is empty, please save some food first")
        else:
            for row in rows:
                storedID.append(int(row[0]))
            for i in range (0,len(storedID)):
                cursor.execute("select Name from ingredient where FID=?",(storedID[i],))
                rows=cursor.fetchall()
                for row in rows:
                    storedName.append(str(row[0]))
            gen(storedID,storedName)
...
def storage(x):
    def ebut_click():
        S[2]="Pronounce the expired ingredients"
        doubleclick(0,0,9)
        if S[3] is True:
            if not expired:
                speak("No ingredients are expired")
            else:
                for i in range(0,len(expired)):
                    speak(f"  {expired[i]}  , date of purchase {pdate1[i]} ,  date of expiry: {edate1[i]}")
    def gbut_click():
        S[2]="Pronounce the about to expire ingredients"
        doubleclick(0,0,10)
        if S[3] is True:
            if not going:
                speak("No ingredients are about to expire")
            else:
                for i in range(0,len(going)):
                    speak(f"  {going[i]}  , date of purchase {pdate2[i]} ,  date of expiry: {edate2[i]}")
    def obut_click():
        S[2]="Pronounce the available ingredients"
        doubleclick(0,0,11)
        if S[3] is True:
            if not ok:
                speak("No ingredients are available")
            else:
                for i in range(0,len(ok)):
                    speak(f"  {ok[i]}  , date of purchase {pdate3[i]} ,  date of expiry: {edate3[i]}")
            
    if x is True:
        storage_window = tk.Toplevel(root,bg="#111111")
        storage_window.title("Food storage check")
        storage_window.geometry("850x650")
        elabel=tk.Label(storage_window,text="Already expired ingredients",fg="#FFFFFF",bg="#111111",font=("Arial",12))
        ebutton=tk.Button(storage_window,text="Pronounce the expired ingredients",font=("Arial",12),command=ebut_click)
        elist=tk.Listbox(storage_window,width=120,fg="#FFFFFF",bg="#111111")
        for i in range (0,len(expired)):
            elist.insert(i,f"Name: {expired[i]} || date of purchase: {pdate1[i]} || date of expiry: {edate1[i]}")
        elabel.grid(row=1,column=0,sticky="w")
        ebutton.grid(row=1,column=0,sticky="e")
        elist.grid(row=2,column=0,sticky="ew")
        glabel=tk.Label(storage_window,text="About to expire ingredients",fg="#FFFFFF",bg="#111111",font=("Arial",12))
        gbutton=tk.Button(storage_window,text="Pronounce the about to expire ingredients",font=("Arial",12),command=gbut_click)
        glist=tk.Listbox(storage_window,width=120,fg="#FFFFFF",bg="#111111")
        for i in range (0,len(going)):
            glist.insert(i,f"Name: {going[i]} || date of purchase: {pdate2[i]} || date of expiry: {edate2[i]}")
        glabel.grid(row=3,column=0,sticky="w")
        gbutton.grid(row=3,column=0,sticky="e")
        glist.grid(row=4,column=0,sticky="ew")
        olabel=tk.Label(storage_window,text="Available ingredients",fg="#FFFFFF",bg="#111111",font=("Arial",12))
        obutton=tk.Button(storage_window,text="Pronounce the available ingredients",font=("Arial",12),command=obut_click)
        olist=tk.Listbox(storage_window,width=120,fg="#FFFFFF",bg="#111111")
        for i in range (0,len(ok)):
            olist.insert(i,f"Name: {ok[i]} || date of purchase: {pdate3[i]} || date of expiry: {edate3[i]}")
        olabel.grid(row=5,column=0,sticky="w")
        obutton.grid(row=5,column=0,sticky="e")
        ...
        while True:
            ...
            if cv2.waitKey(1) & 0xFF == ord('c'):
                ...
                if len(ingredients)==0:
                    speak("No ingredients are detected,  please hold the ingredient infront of the camera again and press C")
                else:
...
label6=tk.Label(frame3,text="(Since expiry dates varies with different external conditions, they are rounded down to ensure food safety)",font=("Arial",10),fg="#FFFFFF",bg="#111111")
...
def button8_click():
    ...
    doubleclick(0,0,8)
    storage(S[3])
button0=tk.Button(frame1,width=38, height=2,text="Start with food expiry management",command=button0_click,bg="#FFFFFF",font=("Arial",13))
button0.grid(row=24,column=2)
button1=tk.Button(frame1,width=38, height=2,text="Start with recipe generation",command=button1_click,bg="#FFFFFF",font=("Arial",13))
button1.grid(row=26,column=2)
button2=tk.Button(frame2,width=30, height=2,text="Return to the previous page",command=button2_click,bg="#FFFFFF",font=("Arial",13))
button3=tk.Button(frame2,width=20, height=2,text="Normal mode",bg="#FFFFFF",command=button3_click,font=("Arial",13))
button4=tk.Button(frame2,width=20, height=2,text="Generate from storage",bg="#FFFFFF",command=button4_click,font=("Arial",13))
button2.grid(row=2,column=2)
button3.grid(row=3,column=2,sticky="e")
button4.grid(row=3,column=2,sticky="w")
button5=tk.Button(frame3,width=30, height=2,text="Return to the previous page",command=button5_click,bg="#FFFFFF",font=("Arial",13))
button6=tk.Button(frame3,width=20, height=2,text="Normal mode",bg="#FFFFFF",command=button6_click,font=("Arial",13))
button7=tk.Button(frame3,width=20, height=2,text="AI mode",bg="#FFFFFF",command=button7_click,font=("Arial",13))
button8=tk.Button(frame3,width=20, height=2,text="Storage check",bg="#FFFFFF",command=button8_click,font=("Arial",13))
